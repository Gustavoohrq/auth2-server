...
module.exports.find = (key, done) => {
  connection.query('SELECT * FROM access_token where token=?', key, function (error, results, fields) {
    if (error) return done(new Error('Token Not Found'))
...
module.exports.findByUserIdAndClientId = (userId, clientId, done) => {
  connection.query('SELECT * FROM access_token where ?', { userId, clientId }, function (error, results, fields) {
    if (error) return done(new Error('Token Not Found'))
    else if (results[0] === '' || results[0] === undefined || results[0] === null) return done(new Error('Token Not Found'))
    return done(null, results[0]);
  })
...
module.exports.save = async (token, userId, clientId, done) => {
  ...
  connection.query('SELECT * FROM access_token where userId=?', userId, function (error, results, fields) {
    const sql = connection.query("INSERT INTO access_token SET ? ", {
      id,
      token,
      created_at,
      userId,
    ...
    if (error) return sql
      ...
      connection.query('DELETE FROM access_token where id = ?', results[0].id, function (erro, results, fields) {
        return sql;
      });
    }
  })
